...
def to_chat(example):
    ...
    return {
        "text": json.dumps(messages),  # list[dict]: [{role, content, ...}, ...]
        "tools": json.dumps(tools),  # list[dict] or []
    }


chat_ds = dataset.map(to_chat, remove_columns=dataset.column_names)
chat_ds = chat_ds.filter(lambda x: x["text"] != "null")
...
if False:
    model.save_pretrained_merged(
        "functiongemma-finetune-16bit", tokenizer, save_method="merged_16bit"
    )
if False:  # Pushing to HF Hub
    model.push_to_hub_merged(
        "hf/functiongemma-finetune-16bit",
        tokenizer,
...
if False:
    model.save_pretrained_merged(
        "functiongemma-finetune-4bit",
        tokenizer,
        save_method="merged_4bit",
    )
if False:  # Pushing to HF Hub
    model.push_to_hub_merged(
        "hf/functiongemma-finetune-4bit", tokenizer, save_method="merged_4bit", token=""
    )

# Just LoRA adapters
if False:
    model.save_pretrained("functiongemma-finetune-lora")
    tokenizer.save_pretrained("functiongemma-finetune-lora")
if False:  # Pushing to HF Hub
    model.push_to_hub("hf/functiongemma-finetune-lora", token="")
    tokenizer.push_to_hub("hf/functiongemma-finetune-lora", token="")
